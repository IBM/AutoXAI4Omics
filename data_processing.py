...
def std_split(x, y, config_dict):
    ...
    # Don't stratify for regression (sklearn can't currently handle it with e.g. binning)
        ...
        try:
            x_train, x_test, y_train, y_test = train_test_split(
                x, y, test_size=test_size, random_state=seed_num
            )
        except:
            print("!!! ERROR: PLEASE SELECT VALID PREDICTION TASK AND TARGET !!!")
            raise
...
def train_eval_feat_selection_model(
    x, y, n_feature, problem_type, eval_model=None, eval_metric=None, method_dict=None
):
    """
    ...
    x_trans, SKB = manual_feat_selection(
        x, y, n_feature, problem_type, method_dict
...
def validate_models_and_metrics(problem_type, estimator, metric):
    ...
    if not (
        ...
        or (
            (problem_type == "classification") and (est._estimator_type == "classifier")
        )
    ):
        raise ValueError(
        ...
        if method_dict["name"] not in globals():
            ...
            if ("estimator" not in method_dict.keys()) or (
                ...
                if (
                    ...
                    (
                        globals()[method_dict["estimator"]]._estimator_type
                        == "classifier"
                    )
