    ...
    def __call__(self, trial):
        ...
        if self.dataset_type == 'classification':
            ...
            for train_index, test_index in kf.split(train_x):
                xgb_model = xgb.XGBClassifier(objective='multi:softmax', num_class=self.num_class, n_jobs=-1, random_state=self.random_state)
...
class XGBoostModel(BaseModel):
    ...
    def __init_fx__(self, input_dim, output_dim, dataset_type):
        ...
        if dataset_type == "classification":
            base_model = xgb.XGBClassifier(max_depth=10,
                                           learning_rate=0.1,
                                           n_estimators=100,
                                           verbosity=0,
                                           silent=True,
            ...
            base_model = xgb.XGBRegressor(objective='reg:squarederror', learning_rate=0.01, max_depth=10,
                                          alpha=10, n_estimators=100, random_state=self.random_state)
            if output_dim > 1:
    ...
    def fit_data_optuna(self, trainX, trainY, testX, testY):
        ...
        objective = XGBoostObjective(self.dataset_type, trainX, trainY, testX, testY, num_class=self.output_dim,random_state=self.random_state)
        ...
        if self.dataset_type == 'classification':
            self.model = xgb.XGBClassifier(objective='multi:softmax', num_class=self.output_dim, random_state=self.random_state, **study.best_params)
        else:
            self.model = xgb.XGBRegressor(objective='reg:squarederror', random_state=self.random_state, **study.best_params)
