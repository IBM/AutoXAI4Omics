...
def filter_biom(config_dict, amp_exp, abundance=10, prevalence=0.01, collapse_tax=None):
    ...
    save_name = (
        f'/experiments/results/{config_dict["data"]["name"]}/omics_{config_dict["data"]["data_type"]}_keptFeatures.pkl'
    )
...
def get_data_microbiome(path_file, metadata_path, config_dict):
    ...
    # save normaliser
    save_name = (
        f'/experiments/results/{config_dict["data"]["name"]}/omics_{config_dict["data"]["data_type"]}_scaler.pkl'
    )
    with open(save_name, "wb") as f:
        joblib.dump(SS, f)
