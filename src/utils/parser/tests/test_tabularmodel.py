...
from ..tabular_model import TabularModel as Model
import pytest
from copy import deepcopy

TEST_CONFIG = {
    "filter_tabular_sample": 0,
    "filter_tabular_measurements": [0, 0],
    "output_file_tab": "data/TEST.csv",
...
class Test_Model:
    ...
    def test_measurementsFieldLength_long(self):
        MODIFIED_CONFIG = deepcopy(TEST_CONFIG)
        MODIFIED_CONFIG["filter_tabular_measurements"] = [0, 0, 0]
        try:
            Model(**MODIFIED_CONFIG)
            assert False
        except Exception as e:
    ...
    def test_measurementsFieldLength_short(self):
        MODIFIED_CONFIG = deepcopy(TEST_CONFIG)
        MODIFIED_CONFIG["filter_tabular_measurements"] = [0]
        try:
            Model(**MODIFIED_CONFIG)
            assert False
        except Exception as e:
    ...
    def test_outputNulling(self):
        MODIFIED_CONFIG = deepcopy(TEST_CONFIG)
        MODIFIED_CONFIG["output_file_tab"] = None
