...
def parser_plotting(plotting_entry,problem):
    ...
    if "plot_method" not in keys:
        ...
        if not given_opts.issubset(max_opts):
            raise ValueError(f'Non-valid plot options given for plot_method: {",".join(list(given_opts-max_opts))}. Possible options: {max_opts}')
...
def parse_autosklearn(sklearnEntry,problem_type):
    ...
    validKeys = {"verbose","estimators","time_left_for_this_task","per_run_time_limit","memory_limit","n_jobs","ensemble_size"}
    ...
    class_opts = ["bernoulli_nb","decision_tree","extra_trees","gaussian_nb","k_nearest_neighbors","lda","liblinear_svc","mlp","multinomial_nb","passive_aggressive","qda", "random_forest"]
    reg_opt = ['adaboost', 'ard_regression', 'decision_tree', 'extra_trees', 'gaussian_process', 'gradient_boosting', 'k_nearest_neighbors', 'liblinear_svr', 'libsvm_svr', 'mlp', 'random_forest', 'sgd']
...
def parse_MLSettings(problemEntry):
    ...
    if "problem_type" not in keys: ###################################### MANDITORY ######################################
        ...
        if problemEntry['problem_type'] not in ['classification','regression']:
            raise ValueError(f'problem_type not valid: {problemEntry["problem_type"]}. Must be "classification" or "regression"')
    ...
    if "hyper_tuning" not in keys:
        ...
        if problemEntry['hyper_tuning'] != None:
            ...
            if problemEntry['hyper_tuning'] not in ["random","grid"]:
                raise ValueError(f'hyper_tuning option invalid: {problemEntry["hyper_tuning"]}. Must be "random" or "grid" or None')
    ...
    if "scorer_list" not in keys:
        ...
        if not given_opts.issubset(max_opts):
            raise ValueError(f'Non-valid options given for scorer_list: {",".join(list(given_opts-max_opts))}. Possible options: {max_opts}')
    
    max_opts = {"rf","adaboost","knn","autoxgboost","autolgbm","autosklearn","autokeras","fixedkeras"}
    if "model_list" not in keys: ###################################### MANDITORY ######################################
        ...
        if problemEntry["model_list"] ==[]:
            ...
            if not given_opts.issubset(max_opts):
                raise ValueError(f'Non-valid options given for model_list: {",".join(list(given_opts-max_opts))}. Possible options: {max_opts}')
        ...
        if ("encoding" not in keys):
            ...
            if problemEntry['encoding'] is not None:
                ...
                if problemEntry['encoding'] not in ['label','onehot']:
                    raise ValueError(f'Encoding entry ({problemEntry["encoding"]}) not valid, must be None or "label" or "onehot"')
...
def parse_data(dataEntry):
    validKeys = {"name","file_path","metadata_file","file_path_holdout_data","metadata_file_holdout_data","save_path","target","data_type"}
...
def parse_microbiome(omicEntry):
    validKeys = {"collapse_tax","min_reads","norm_reads","filter_abundance","filter_prevalence","filter_microbiome_samples","remove_classes","merge_classes"}
    ...
    if 'collapse_tax' not in keys:
        ...
        if omicEntry['collapse_tax']!= None:
            ...
            if omicEntry['collapse_tax'] not in ['genus', 'species']:
                raise ValueError(f'microbiome:collapse_tax option not valid: {omicEntry["collapse_tax"]}. Valid options: {["genus", "species"]}')
...
def parse_geneExpression(omicEntry):
    ...
    if 'expression_type' not in keys:
        raise ValueError(f'expression_type must be given and one of: "FPKM", "RPKM", "TMM", "TPM", "Log2FC", "COUNTS", "OTHER"' )
    else:
        if omicEntry['expression_type']!= None:
            ...
            if omicEntry['expression_type'] not in valid:
                raise ValueError(f'geneExpression:expression_type option not valid: {omicEntry["expression_type"]}. Valid options: {valid}')
