    ...
    if "shap_plots" in plotting_entry["plot_method"]:
        ...
        if "explanations_data" not in keys:
            ...
            if plotting_entry["explanations_data"] not in ["test", "exemplars", "all"]:
                raise ValueError(
                    f'explanations_data must one of: "test", "exemplars", "all"'
                )
...
def parse_autosklearn(sklearnEntry, problem_type):
    ...
    if "time_left_for_this_task" not in keys:
        ...
        if sklearnEntry["time_left_for_this_task"] < 1:
            raise ValueError(
                f"autosklearn:time_left_for_this_task must be an int greater than 0"
            )
    ...
    if "per_run_time_limit" not in keys:
        ...
        if sklearnEntry["per_run_time_limit"] < 1:
            raise ValueError(
                f"autosklearn:per_run_time_limit must be an int greater than 0"
            )
    ...
    if "memory_limit" not in keys:
        ...
        if sklearnEntry["memory_limit"] != None:
            ...
            if sklearnEntry["memory_limit"] < 0:
                raise ValueError(
                    f"autosklearn:memory_limit must be None or an int greater than 0"
                )
...
def parse_MLSettings(problemEntry):
    ...
    if (
        ...
        raise ValueError(
            f'problem_type must be given. Options: "classification" or "regression"'
        )
    else:
    ...
    if "fit_scorer" not in keys:
        problemEntry["fit_scorer"] = (
            "f1" if problemEntry["problem_type"] == "classification" else "mean_ape"
        )
    else:
    ...
    if "scorer_list" not in keys:
        problemEntry["scorer_list"] = (
            classif if problemEntry["problem_type"] == "classification" else reg
        )
    else:
    ...
    if (
        ...
        raise ValueError(
            f"model_list must be a list containg at least one option from: {max_opts}"
        )
    else:
        ...
        if problemEntry["model_list"] == []:
            raise ValueError(
                f"model_list must contain at least one option from: {max_opts}"
            )
        else:
    ...
    if "min_reads" not in keys:
        ...
        if omicEntry["min_reads"] != None:
            ...
            if omicEntry["min_reads"] < 0:
                raise ValueError(
                    f"microbiome:min_reads entry must be an int greater than 0"
                )
    ...
    if "norm_reads" not in keys:
        ...
        if omicEntry["norm_reads"] != None:
            ...
            if omicEntry["norm_reads"] < 0:
                raise ValueError(
                    f"microbiome:norm_reads entry must be an int greater than 0"
                )
    ...
    if "filter_abundance" not in keys:
        ...
        if omicEntry["filter_abundance"] != None:
            ...
            if omicEntry["filter_abundance"] < 0:
                raise ValueError(
                    f"microbiome:filter_abundance entry must be an int greater than 0"
                )
    ...
    if "filter_prevalence" not in keys:
        ...
        if omicEntry["filter_prevalence"] != None:
            ...
            if (omicEntry["filter_prevalence"] < 0) or (
                ...
                raise ValueError(
                    f"microbiome:filter_prevalence must be a float between 0 and 1"
                )
    ...
    if "filter_genes" not in keys:
        ...
        if omicEntry["filter_genes"] != None:
            ...
            if len(omicEntry["filter_genes"]) != 2:
                raise ValueError(
                    f"filter_genes must be a list of 2 integers greater than 0."
                )
        ...
        if omicEntry["filter_metabolomic_sample"] != None:
            try:
                ...
                try:
                    ...
                    raise ValueError(
                        f"filter_metabolomic_sample must be a int or a float >0"
                    )
    ...
    if "filter_measurements" not in keys:
        ...
        if omicEntry["filter_measurements"] != None:
            ...
            if len(omicEntry["filter_measurements"]) != 2:
                raise ValueError(
                    f"filter_measurements must be a list of 2 integers greater than 0."
                )
        ...
        if omicEntry["filter_tabular_sample"] != None:
            try:
                ...
                try:
                    ...
                    raise ValueError(
                        f"filter_tabular_sample must be a int or a float >0"
                    )
            if omicEntry["filter_tabular_sample"] < 0:
                raise ValueError(
                    f"filter_tabular_sample must be greater than or equal to 0."
                )
    ...
    if "filter_tabular_measurements" not in keys:
        ...
        if omicEntry["filter_tabular_measurements"] != None:
            ...
            if len(omicEntry["filter_tabular_measurements"]) != 2:
                raise ValueError(
                    f"filter_tabular_measurements must be a list of 2 integers greater than 0."
                )
