        ...
        if isinstance(value, list):
            ...
            if value[0] == "range":
                ...
                if isinstance(value[0], int):
                    ...
                    param_ranges[key] = [
                        int(i) for i in np.linspace(start, stop, num=num_vals)
                    ]
                # Same as above but using gloats
                elif isinstance(value[0], float):
                    start, stop, num_vals = value
                    param_ranges[key] = [
                        float(i) for i in np.linspace(start, stop, num=num_vals)
                    ]
            # Nothing needs to be done with a list of strings
                ...
                raise TypeError(
                    f"{value} of type {type(value)} is not a valid parameter in a list"
                )
...
def define_scorers(problem_type):
    ...
    if problem_type == "classification":
        ...
        scorer_dict = {
            ...
            "mean_ae": skm.make_scorer(
                skm.mean_absolute_error, greater_is_better=False
            ),
            "med_ae": skm.make_scorer(
                skm.median_absolute_error, greater_is_better=False
            ),
            "rmse": skm.make_scorer(rmse, greater_is_better=False),
            "mean_ape": skm.make_scorer(
                skm.mean_absolute_percentage_error, greater_is_better=False
            ),
            "r2": skm.make_scorer(skm.r2_score, greater_is_better=True),
        }
    else:
        raise ValueError(
            f"{problem_type} is not recognised, must be either 'regression' or 'classification'"
        )
    return scorer_dict
...
def evaluate_model(model, problem_type, x_train, y_train, x_test, y_test):
    ...
    if problem_type == "classification":
        ...
        score_dict = {
            ...
            "Precision_PerClass_Train": precision_score(
            ...
            "ROC_auc_score_Train": roc_auc_score(
                y_train, pred_train_proba, multi_class="ovo"
            ),
            "ROC_auc_score_Test": roc_auc_score(
                y_test, pred_test_proba, multi_class="ovo"
            ),
...
def run_models(
    ...
    if config_dict["data"]["data_type"] == "microbiome":
        ...
        if config_dict["microbiome"]["collapse_tax"] is not None:
            fname += str(config_dict["microbiome"]["collapse_tax"])
